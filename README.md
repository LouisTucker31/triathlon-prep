...
- Everything you enter (ticks, race details, emergency contact) is stored only
  ...
  "Reset app" in settings clears it.
- The one exception: text typed into **Venue** is sent to
  [Photon](https://photon.komoot.io) (run by komoot, using OpenStreetMap data)
  to suggest places. It is sent with no referrer and no cookies.
...
- **Content Security Policy** is set with a `<meta>` tag in `index.html`. Scripts,
  ...
  `data:` (used by the nav's glass effect); network requests allow this site and
  `https://photon.komoot.io`. If you add a new external service, add it to
  `connect-src`.
...
```
Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self' https://photon.komoot.io; manifest-src 'self'; worker-src 'self'; object-src 'none'; base-uri 'self'; form-action 'none'; frame-ancestors 'none'
